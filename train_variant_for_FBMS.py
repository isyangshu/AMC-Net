...
def trainIters(args):
    ...
    for e in range(epoch_resume, args.max_epoch):
        print("Epoch", e)
        epoch_losses = {'train': {'iou': [], 'iou_single': [], 'output':[], 'mask_loss': []},
                        'val': {'iou': [], 'iou_single': [], 'output':[], 'mask_loss': []}}
        ...
        for split in ['train', 'val']:
            ...
            for batch_idx, (image, flow, mask) in enumerate(loaders[split]):
                ...
                if split == 'train':
                    ...
                    output, output_single, c4_attention, c3_attention, c2_attention, c1_attention, c0_attention = Model(image, flow)
                    loss_output = criterion(output, mask)
                    loss_single = criterion(output_single, mask)
                    ...
                    loss = loss_output + loss_single + loss_c4 + loss_c3 + loss_c2 + loss_c1 + loss_c0
                    iou = db_eval_iou_multi(mask.cpu().detach().numpy(),
                                            output.cpu().detach().numpy())
                    iou_single = db_eval_iou_multi(mask.cpu().detach().numpy(),
                                            output_single.cpu().detach().numpy())
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    ...
                    with torch.no_grad():
                        output, output_single, c4_attention, c3_attention, c2_attention, c1_attention, c0_attention = Model(
                            image, flow)
                        loss_output = criterion(output, mask)
                        loss_single = criterion(output_single, mask)
                        loss_c4 = criterion(c4_attention,
                ...
                epoch_losses[split]['iou_single'].append(iou_single)
                ...
                if (batch_idx + 1) % args.print_every == 0:
                    ...
                    msiou = np.mean(epoch_losses[split]['iou_single'])

                    te = time.time() - start
                    log = 'Epoch: [{}/{}][{}/{}]\tLr {:.5f}\tTime {:.3f}s''\tMask Loss: {:.4f}\toutput Loss: {:.4f}\tIOU: {:.4f}\tIOU_single: {:.4f}'.format(e, args.max_epoch, batch_idx,
                                                 len(loaders[split]), optimizer.param_groups[0]['lr'], te, mmask, mout, miou, msiou)
                    print(log)
                    open(log_path, 'a').write(log + '\n')
                    start = time.time()
        miou = np.mean(epoch_losses['val']['iou'])
        msiou = np.mean(epoch_losses['val']['iou_single'])
        if e % 5 == 0:
