...
def _read_and_eval_file(mask_video_path: str, pred_video_path: str, frame_name: str):
    ...
    frame_pred_path = os.path.join(pred_video_path, frame_name[:-3]+'png')
    ...
    binary_frame_mask = (frame_mask > 127).astype(np.float32)
...
def _eval_video_sequence(
    ...
    mask_video_path = os.path.join(mask_data_root, video_name, 'shots/001/labels')
...
def eval_method_from_data(
    ...
    eval_video_name_list = os.listdir(mask_data_root)
