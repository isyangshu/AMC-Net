...
class DavisDataset(Dataset):
    ...
    default_hyper_params = dict(
        dataset_root="/workdir/AMCNet/datasets/DAVIS/", subsets=["train2016"], ratio=1.0, max_diff=10,
    )

    def __init__(self, subsets=["train2016"]):
        r"""
    ...
    def _ensure_cache(self):
        ...
        for subset in self.default_hyper_params["subsets"]:
            ...
            flow_root = osp.join(dataset_root, "Flow", "480p")
            if year == "2016":
...
class DavisDatasetVal(Dataset):
    ...
    data_items = []
    default_hyper_params = dict(
        dataset_root="/workdir/AMCNet/datasets/DAVIS/", subsets=["val2016"], ratio=1.0, max_diff=10,
    )

    def __init__(self, subsets=["val2016"]):
        r"""
        Create davis dataset
        """
        super(DavisDatasetVal, self).__init__()
        dataset_root = self.default_hyper_params["dataset_root"]
        self.default_hyper_params["subsets"] = subsets
        self.default_hyper_params["dataset_root"] = osp.realpath(dataset_root)
        if len(DavisDatasetVal.data_items) == 0:
            self._ensure_cache()
    ...
    def __getitem__(self, item):
        ...
        :return:
            ...
            meta (optional)
        """
        record = DavisDatasetVal.data_items[item]
        anno = [[anno_file, record['obj_id']] for anno_file in record["annos"]]
        sequence_data = dict(image=record["image_files"], anno=anno, flow=record['flow'])
        return sequence_data

    def __len__(self):
        return len(DavisDatasetVal.data_items)
    ...
    def _ensure_cache(self):
        ...
        for subset in self.default_hyper_params["subsets"]:
            ...
            if osp.exists(cache_file):
                with open(cache_file, 'rb') as f:
                    DavisDatasetVal.data_items += pickle.load(f)
                logger.info("{}: loaded cache file {}".format(
                    DavisDatasetVal.__name__, cache_file))
            else:
                meta_file = osp.join(dataset_root, "ImageSets", year,
                                     subset[:-4] + ".txt")
                with open(meta_file) as f:
                ...
                logger.info(
                    "Davis VOS dataset: cache dumped at: {}".format(cache_file))
                DavisDatasetVal.data_items += data_anno_list