...
import numpy as np
import torch
import datasets.dataloader_list.helpers as helpers
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            if 'meta' in elem:
                continue
            tmp = sample[elem]
            if elem == 'ref_img' or elem == 'curr_img' or elem == 'prev_img' or elem == 'flow' or elem == 'depth':
...
#        while (not is_contain_obj) and (step < 5):
        if self.step is None:
            while not is_contain_obj:
                ...
                new_ref_label=ref_label[top: top + new_h, left:left + new_w]
                if len(np.unique(new_ref_label)) == 1:
                    continue
                else:
                    for elem in sample.keys():
                        if 'meta' in elem:
            ...
            while not is_contain_obj and st<self.step:
                ...
                ref_label=sample['ref_label']
                new_ref_label=ref_label[top: top + new_h, left:left + new_w]
                if len(np.unique(new_ref_label)) == 1 or st < self.step-1:
                    continue
                else:
                    for elem in sample.keys():
                        if 'meta' in elem:
