...
class AMCNet(nn.Module):
    ...
    def forward(self, input, flow):
        ...
        temp_RGB = {'RGB_G0': G0[:, 0, :, :].unsqueeze(1), 'RGB_G1': G1[:, 0, :, :].unsqueeze(1),
                    'RGB_G2': G2[:, 0, :, :].unsqueeze(1), 'RGB_G3': G3[:, 0, :, :].unsqueeze(1),
                    'RGB_G4': G4[:, 0, :, :].unsqueeze(1)}
        temp_Flow = {'Flow_G0': G0[:, 1, :, :].unsqueeze(1), 'Flow_G1': G1[:, 1, :, :].unsqueeze(1),
                     'Flow_G2': G2[:, 1, :, :].unsqueeze(1), 'Flow_G3': G3[:, 1, :, :].unsqueeze(1),
                     'Flow_G4': G4[:, 1, :, :].unsqueeze(1)}
        if self.training:
            return output, c4_attention, c3_attention, c2_attention, c1_attention, c0_attention
        return output, temp_RGB, temp_Flow
